...
export default function Step2_Identify({ token, onNext, onBack, onIdentified }) {
  ...
  const handleIdentify = async () => {
    ...
    setError('');
    try {
      // Chama o endpoint para identificar o signatário (salvar CPF, etc.)
      await api.post(`/sign/${token}/identify`, { cpf: formData.cpf, phone: formData.phone });
      
      // Chama o endpoint para iniciar o envio do OTP
      await api.post(`/sign/${token}/otp/start`);
      
      onIdentified(formData); // Salva os dados para o próximo passo
      onNext(); // Vai para o passo de verificação de OTP
    } catch (err) {
  ...
  return (
    <Card className="w-full bg-white shadow-lg rounded-xl border-none p-8">
      <CardHeader className="text-center p-0 mb-8">
        ...
        <p className="text-muted-foreground mt-2">Confirme sua identidade para assinar o documento</p>
      </CardHeader>
      <CardContent className="p-0 space-y-5">
        <AuthInput id="name" label="Nome" required onChange={handleChange} />
        <AuthInput id="email" label="Email" type="email" required onChange={handleChange} />
